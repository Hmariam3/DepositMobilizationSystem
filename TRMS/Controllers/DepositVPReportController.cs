...
{
    public class DepositVPReportController : Controller
    {
        private TRMSEntities db = new TRMSEntities();

        [AuthorizeRoles("Admin", "Maker", "District", "SuperUser")]
        // GET: Transactions with Filters
        public ActionResult VPPlanReport(DateTime? startDate, DateTime? endDate)
        {

            string VPUser = Session["Process"].ToString();
            string userPosition = Session["Position"].ToString();
            var transactions = db.DepositPlans.AsQueryable();
            decimal totalAmount = 0;

            if (userPosition == "VP" && !string.IsNullOrEmpty(VPUser))
            {
                transactions = transactions.Where(t => t.Process.Trim() == VPUser.Trim());
                totalAmount = transactions.Where(t => t.Process.Trim() == VPUser.Trim()).Select(t => (decimal?)t.AccountBalance - t.IntialAccountBalance + t.Amount).Sum() ?? 0m;
        ...
        [AuthorizeRoles("Admin", "Maker", "District", "SuperUser")]
        // GET: Transactions with Filters
        public ActionResult VPAgenetReport(DateTime? startDate, DateTime? endDate)
        {
            string VPUser = Session["Process"].ToString();
            string userPosition = Session["Position"].ToString();
            ...
            {
                transactions = transactions.Where(t => t.Process.Trim() == VPUser.Trim());
                totalAmount = transactions.Where(t => t.Process.Trim() == VPUser.Trim()).Select(t => (decimal?)t.AccountBalance - t.IntialAccountBalance + t.Target).Sum() ?? 0m;
        ...
        [AuthorizeRoles("Admin", "Maker", "District", "SuperUser")]
        // GET: Transactions with Filters
        public ActionResult VPFcyReport(DateTime? startDate, DateTime? endDate)
        {
            string VPUser = Session["Process"].ToString();
            string userPosition = Session["Position"].ToString();
            ...
            {
                transactions = transactions.Where(t => t.Process.Trim() == VPUser.Trim());
                totalAmount = transactions.Where(t => t.Process.Trim() == VPUser.Trim()).Select(t => (decimal?)t.TransactionAmount).Sum() ?? 0m;
        ...
        public ActionResult ExportDepositPlan(DateTime? startDate, DateTime? endDate)
        {
            ...
            if (!string.IsNullOrEmpty(VPUser))
            {
                transactions = transactions.Where(t => t.Process.Trim() == VPUser.Trim());
                totalAmount = transactions.Where(t => t.Process.Trim() == VPUser.Trim()).Select(t => (decimal?)t.Amount).Sum() ?? 0m;
            ...
            {
                var ws = wb.Worksheets.Add("Process Deposit Report"); // Add sheet
                ...
                return File(stream, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Process Report.xlsx");
            }
        ...
        public ActionResult ExportDepositAgent(DateTime? startDate, DateTime? endDate)
        {
            ...
            {
                transactions = transactions.Where(t => t.Process.Trim() == VPUser.Trim());
                totalAmount = transactions.Where(t => t.Process.Trim() == VPUser.Trim()).Select(t => (decimal?)t.Target).Sum() ?? 0m;
            ...
            using (var wb = new XLWorkbook()) // Create workbook
            {
                var ws = wb.Worksheets.Add("Process Merchant Report"); // Add sheet
                ...
                return File(stream, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Process Merchant Report.xlsx");
            }
        ...
        public ActionResult ExportDepositFcy(DateTime? startDate, DateTime? endDate)
        {
            ...
            using (var wb = new XLWorkbook()) // Create workbook
            {
                var ws = wb.Worksheets.Add("Process  Fcy Report"); // Add sheet
                ...
                return File(stream, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Process Fcy Report.xlsx");
            }
