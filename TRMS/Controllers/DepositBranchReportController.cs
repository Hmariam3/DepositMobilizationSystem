...
{
    public class DepositBranchReportController : Controller
    {
        ...
        public ActionResult BranchDeposit(DateTime? startDate, DateTime? endDate)
        {
            string branchUser = Session["UserHomeBranch"].ToString();
            string userPosition = Session["Position"].ToString();
            var transactions = db.DepositPlans.AsQueryable();
            decimal totalAmount = 0;

            if ((userPosition == "Manager" || userPosition == "BranchManager") && !string.IsNullOrEmpty(branchUser))
            {
                transactions = transactions.Where(t => t.Branch.Trim() == branchUser.Trim());
                totalAmount = transactions.Where(t => t.Branch.Trim() == branchUser.Trim()).Select(t => (decimal?)t.AccountBalance - t.IntialAccountBalance).Sum() ?? 0m;
        ...
        [AuthorizeRoles("Admin", "Maker", "SuperUser")]
        // GET: Transactions with Filters
        public ActionResult BranchMerchant(DateTime? startDate, DateTime? endDate)
        {
            string branchUser = Session["UserHomeBranch"].ToString();
            string userPosition = Session["Position"].ToString();
        ...
        [AuthorizeRoles("Admin", "Maker", "SuperUser")]
        // GET: Transactions with Filters
        public ActionResult BranchFCY(DateTime? startDate, DateTime? endDate)
        {
            string branchUser = Session["UserHomeBranch"].ToString();
            string userPosition = Session["Position"].ToString();
            ...
            {
                transactions = transactions.Where(t => t.Branch.Trim() == branchUser.Trim());
                totalAmount = transactions.Where(t => t.Branch.Trim() == branchUser.Trim()).Select(t => (decimal?)t.TransactionAmount).Sum() ?? 0m;
        ...
        public ActionResult ExportToExcel(DateTime? startDate, DateTime? endDate)
        {
            ...
            decimal totalAmount = 0;
            if (!string.IsNullOrEmpty(branchUser))
            {
                transactions = transactions.Where(t => t.Branch == branchUser);
            }
        ...
        public ActionResult ExportDepositAgent(DateTime? startDate, DateTime? endDate)
        {
            ...
            {
                transactions = transactions.Where(t => t.Branch.Trim() == branchUser.Trim());
                totalAmount = transactions.Where(t => t.Branch.Trim() == branchUser.Trim()).Select(t => (decimal?)t.Target).Sum() ?? 0m;
